...
/*
1376. 通知所有员工所需的时间
公司里有 n 名员工，每个员工的 ID 都是独一无二的，编号从 0 到 n - 1。公司的总负责
人通过 headID 进行标识。
在 manager 数组中，每个员工都有一个直属负责人，其中 manager[i] 是第 i 名员工的
直属负责人。对于总负责人，manager[headID] = -1。题目保证从属关系可以用树结构显示。
公司总负责人想要向公司所有员工通告一条紧急消息。他将会首先通知他的直属下属们，然后
由这些下属通知他们的下属，直到所有的员工都得知这条紧急消息。
...
func numOfMinutes(n int, headID int, manager []int, informTime []int) int {
	...
	for i, v := range manager {
		if _, ok := minuteMap[v]; ok {
			minuteMap[v] = append(minuteMap[v], i)
		} else {
			minuteMap[v] = []int{i}
		}
	}
	output := 0
	cost := 0
	var trackback func(headID int)
	trackback = func(headID int) {
		down, ok := minuteMap[headID]
		if !ok {
			if cost > output {
				output = cost
			}
...
func main()  {
	fmt.Println(numOfMinutes(7, 6, []int{1, 2, 3, 4, 5, 6, -1},
	[]int{0, 6, 5, 4, 3, 2, 1}))
}