...
设 m 为数组的中位数，只要满足下述两个前提之一，就可以判定 arr[i] 的值比 arr[j]
的值更强：
 |arr[i] - m| > |arr[j] - m|
...
输出：[5,1]
解释：中位数为 3，按从强到弱顺序排序后，数组变为 [5,1,4,2,3]。最强的两个元素
是 [5, 1]。[1, 5] 也是正确答案。
注意，尽管 |5 - 3| == |1 - 3| ，但是 5 比 1 更强，因为 5 > 1 。
示例 2：
输入：arr = [1,1,3,5,5], k = 2
输出：[5,5]
解释：中位数为 3, 按从强到弱顺序排序后，数组变为 [5,5,1,1,3]。最强的两个元素
是 [5, 5]。
...
func getStrongest(arr []int, k int) []int {
	...
	sort.Slice(arr, func(i, j int) bool {
		if abs1471(arr[i] - m) == abs1471(arr[j] - m) {
			return arr[i] > arr[j]
		}
		return abs1471(arr[i] - m) > abs1471(arr[j] - m)
	})
	return arr[:k]
}

func abs1471(a int) int {
	if a < 0 {
		return -a
	}
	return a
